...
# Activate venv (aiperf installed under Python 3.13)
...
# -- SKU helpers (bash 3.2 compatible) ----------------------------------------
sku_endpoint_name() {
  case "$1" in
...
# We fetch credentials for all requested SKUs up front (sequential az calls)
# and store them in plain variables keyed by SKU name.
declare_creds() {
  local sku="$1"
  local ep_name
  ...
  # Export as SKU-namespaced vars (e.g. BASE_URL_a100, API_KEY_a100)
...
if [[ -n "${E2E_LOG_DIR:-}" ]]; then
  ...
  BENCH_DIR="$SCRIPT_DIR/../../logs/cli/benchmark_$(date '+%Y-%m-%d_%H-%M-%S')"
fi
mkdir -p "$BENCH_DIR"

# -- Download tokenizer from model blob storage (not HF Hub) ------------------
# Discover model reference from the first deployment, resolve its blob path in
# the registry, list files, and download only the tokenizer files.
...
# Get Azure Storage bearer token and list/download tokenizer files
...
" <<< "$BLOB_LIST_XML" 2>/dev/null || true
    ...
    # Download only the tokenizer-related files
...
if [[ "$TOKENIZER_DOWNLOADED" == true ]]; then
  ...
  LOCAL_ARTIFACTS="$(cd "$SCRIPT_DIR/../../.." && pwd)/model-artifacts"
  if [[ -f "$LOCAL_ARTIFACTS/tokenizer.json" ]]; then
    TOKENIZER="$LOCAL_ARTIFACTS"
...
PLOT_SCRIPT="$SCRIPT_DIR/../../scripts/plot-benchmark.py"
if [[ -f "$PLOT_SCRIPT" ]]; then
  info "Generating custom benchmark charts..."
  python "$PLOT_SCRIPT" "$BENCH_DIR"
  info "Custom charts saved to: $BENCH_DIR/plots/"
else
  info "plot-benchmark.py not found at $PLOT_SCRIPT -- skipping custom charts."
fi

# -- Summary ------------------------------------------------------------------
echo ""
echo "======================================================================"
echo "  BENCHMARK COMPLETE (${SKUS[*]})"
echo "======================================================================"
