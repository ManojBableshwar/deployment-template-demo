...
#       See bugs/env-image-api-not-exposed.md for details.
if az ml environment show --name "$ENVIRONMENT_NAME" --version "$ENVIRONMENT_VERSION" \
...
# Fetch the built Docker image details (ACR address, image URI, Dockerfile)
# NOTE: The ARM API / CLI do not expose the built image URI or ACR address.
#       We must call the Studio internal environment image API to get this info.
#       See bugs/env-image-api-not-exposed.md for details.
info "Fetching built Docker image details from environment image API..."
ENV_IMAGE_API="https://ml.azure.com/api/${WORKSPACE_LOCATION}/environment/v1.0/subscriptions/${SUBSCRIPTION_ID}/resourceGroups/${RESOURCE_GROUP}/providers/Microsoft.MachineLearningServices/workspaces/${AZUREML_WORKSPACE}/environments/${ENVIRONMENT_NAME}/versions/${ENVIRONMENT_VERSION}/image?secrets=false"
TOKEN=$(az account get-access-token --query accessToken -o tsv 2>/dev/null)
IMAGE_DETAILS=$(curl -s -H "Authorization: Bearer $TOKEN" "$ENV_IMAGE_API" 2>/dev/null || true)
if [[ -n "$IMAGE_DETAILS" ]]; then
  ACR_ADDRESS=$(echo "$IMAGE_DETAILS" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('dockerImage',{}).get('registry',{}).get('address',''))" 2>/dev/null || true)
  DOCKER_IMAGE=$(echo "$IMAGE_DETAILS" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('dockerImage',{}).get('name',''))" 2>/dev/null || true)
  info "ACR address:  ${ACR_ADDRESS:-<not available>}"
  info "Docker image: ${DOCKER_IMAGE:-<not available>}"
  echo ""
  echo "Environment image details:"
  echo "$IMAGE_DETAILS" | python3 -m json.tool 2>/dev/null || echo "$IMAGE_DETAILS"
fi
